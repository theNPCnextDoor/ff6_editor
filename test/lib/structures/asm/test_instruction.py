...
from src.lib.structures.asm.instruction import Instruction, NoCandidateException, BranchingInstruction
...
class TestInstruction:
    ...
    def test_command(self, instruction: Instruction, is_flag_setter: bool):
    ...
    @pytest.mark.parametrize(
        ["instruction", "input", "output"], [
            ...
            (Instruction(opcode=Bytes("E2"), data=Bytes("00")), Flags(m=False, x=False),
             Flags(m=False, x=False)),
            (Instruction(opcode=Bytes("E2"), data=Bytes("10")), Flags(m=False, x=False),
             Flags(m=False, x=True)),
            (Instruction(opcode=Bytes("E2"), data=Bytes("20")), Flags(m=False, x=False),
             Flags(m=True, x=False)),
            (Instruction(opcode=Bytes("E2"), data=Bytes("3F")), Flags(m=False, x=False),
             Flags(m=True, x=True)),
            (Instruction(opcode=Bytes("E2"), data=Bytes("30")), Flags(m=True, x=True),
             Flags(m=True, x=True))
        ]
    )
...
class TestBranchingInstruction:
    ...
    def test_is_destination_possible(self, command: str, position: Position, destination: Position, expected_result: bool):
        assert BranchingInstruction.is_destination_possible(
            position=position, destination=destination, command=command
        ) == expected_result
    ...
    @pytest.mark.parametrize(
        ["instruction", "show_address", "output"], [
            (BranchingInstruction(position=Position(0x120000), opcode=Bytes(0x4C), destination=Position(0x123456)), True, "  JMP label_1 # D2/0000"),
            (BranchingInstruction(position=Position(0x120000), opcode=Bytes(0x4C), destination=Position(0x123457)), True, "  JMP $5734 # D2/0000"),
            (BranchingInstruction(position=Position(0x120000), opcode=Bytes(0x4C), destination=Position(0x123456)), False,
            "  JMP label_1"),
            (
            BranchingInstruction(position=Position(0x120000), opcode=Bytes(0x4C), destination=Position(0x123457)), False,
            "  JMP $5734")
