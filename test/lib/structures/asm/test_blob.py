...
class TestBlob:
    ...
    @pytest.mark.parametrize(
        ["line", "blob"],
        [
            ("  $12", Blob(data=BlobBytes(0x12))),
            ("  $0123456789ABCDEF", Blob(data=BlobBytes(0xEFCDAB8967452301))),
            ("  $FFFF # C0/0000", Blob(data=BlobBytes(0xFFFF))),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ["blob", "expected"],
        [
            (Blob(data=BlobBytes("12345678")), "$12345678"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (Blob(data=BlobBytes("12345678")), False, "  $12345678"),
            (Blob(data=BlobBytes("12345678")), True, "  $12345678 # C0/0000"),
        ],
    )
