    ...
    def __set__(self, instance, value):
        ...
        if instance.available_space < len(new_message):
            ...
            while overflow > 0:
                ...
                overflow = overflow - (next_dialog.available_space - len(compressed_message))
                i += 1
        for i, message in enumerate(compressed_messages):
            ...
            if next_dialog.id <= RomMap.DIALOG_QUANTITY:
                ...
                if next_dialog.pointer < current_dialog.pointer:
                    Binary()[RomMap.DIALOG_POINTER_2ND_BANK : RomMap.DIALOG_POINTER_2ND_BANK + 2] = bytes(
                        Bytes(next_dialog.id, length=2)
                    )
            if current_dialog.address + len(message) > RomMap.DIALOG_2ND_BANK_END:
                raise IndexError(f"The dialog goes over the limit {RomMap.DIALOG_2ND_BANK_END:06X}.")
            Binary()[current_dialog.address : current_dialog.address + len(message)] = bytes(message)
